...
export default function ProductsWhatsAppShareButton({
  ...
  const handleClick = async (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    try {
      const shared = await shareTextWithOptionalImage(msg, shareImageUrls, 'products.jpg')
      if (shared) return
    } finally {
      setSharing(false)
    }
  ...
  return (
    <a
      href="#"
      onClick={handleClick}
      aria-label="Share this product list on WhatsApp"
      aria-busy={sharing}
      className="flex w-full max-w-md mx-auto items-center justify-center gap-2 rounded-full bg-[#25D366] py-3 px-6 text-base font-medium text-white transition-colors hover:bg-[#20bd5a] active:scale-95 disabled:opacity-70"
    >
